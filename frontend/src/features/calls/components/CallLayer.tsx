...
import { averageGradientColors, getReadableTextColor } from "@/components/messenger/utils/color";
...
export function CallLayer({
    ...
    const [floatingPosition, setFloatingPosition] = useState<{ left: number; top: number } | null>(
        null,
    );
    ...
    const isGroupCall = Boolean(context.session?.is_group);
    const peer = context.peer;
    const active = ["connecting", "connected", "reconnecting"].includes(state);
    const timer = useCallTimer(state === "connected", context.session?.started_at);
    ...
    const controlsTextColor =
        ...
        (controlsBackground
            ? getReadableTextColor(averageGradientColors(controlsBackground) ?? controlsBackground)
            : muiTheme.palette.text.primary);

    const groupName = context.conversation?.name || "Nhóm";
    ...
    if (
        ...
        const displayName = isGroupCall
            ? groupName
            : displayNameOf(peer, context.isCaller ? "Đang gọi" : "Cuộc gọi đến");
        return (
            <Paper
                elevation={12}
                role="button"
            ...
            >
                {!isGroupCall && <StreamAudio stream={remoteStream} />}
                <PeerAvatar
                    peer={peer}
                    size={44}
                    groupAvatar={isGroupCall ? peerAvatar : undefined}
                />
    ...
    // -------------------------------------------------------------------------
    // Incoming ring dialog – group call
    // -------------------------------------------------------------------------
    if (state === "ringing" && !context.isCaller && isGroupCall) {
        const callerPart = [...context.groupParticipants.values()].find(
            (p) => p.user_id === context.session?.caller_id,
        );
        return (
            <Dialog
                open
                ...
                slotProps={{
                    paper: { sx: { bgcolor: "#070b18", color: "#fff", overflow: "hidden" } },
                }}
            >
                <DialogContent sx={{ position: "relative", overflow: "hidden" }}>
                    ...
                    >
                        <Box sx={{ position: "relative" }}>
                            <Avatar
                                src={resolveCdnUrl(callerPart?.avatar)}
                        ...
                        <Box textAlign="center">
                            ...
                            <Typography sx={{ color: "rgba(255,255,255,0.76)" }}>
                                {callerPart?.fullname || "Ai đó"} đang gọi nhóm •{" "}
                                {callType === "audio" ? "Cuộc gọi thoại" : "Cuộc gọi video"}
                            </Typography>
                            {context.groupParticipants.size > 0 && (
                                <Typography
                                    variant="caption"
                                    sx={{ color: "rgba(255,255,255,0.56)" }}
                                >
                                    {context.groupParticipants.size} người trong phòng
                                </Typography>
                            )}
                        </Box>
            ...
            >
                <DialogContent sx={{ position: "relative", overflow: "hidden" }}>
                    ...
                    >
                        ...
                        <Box textAlign="center">
                            <Typography variant="h6" fontWeight={800}>
                                {displayNameOf(peer, "Cuộc gọi đến")}
                            </Typography>
                            <Typography sx={{ color: "rgba(255,255,255,0.76)" }}>
    ...
    // -------------------------------------------------------------------------
    // Outgoing call / group calling state
    // -------------------------------------------------------------------------
    if ((state === "calling" || state === "permission_checking") && context.isCaller) {
        const name = isGroupCall ? groupName : displayNameOf(peer, "Đang gọi");
        if (isAudioCall) {
            return (
            ...
            <Dialog
                ...
                slotProps={
                    isAudioCall
                        ? {
                              paper: {
                                  sx: { bgcolor: "#070b18", color: "#fff", overflow: "hidden" },
                              },
                          }
                        : undefined
                }
    ...
    // -------------------------------------------------------------------------
    // Floating video PiP (collapsed, video, 1-on-1)
    // -------------------------------------------------------------------------
    ...
    if (collapsed && !isAudioCall && !isGroupCall) {
        return (
            <Paper
                ref={floatingRef}
                elevation={18}
    ...
    // Floating group call PiP
    if (collapsed && isGroupCall) {
        const participants = [...context.groupParticipants.values()];
        ...
        return (
            <Paper
                ...
                role="button"
                tabIndex={0}
                aria-label="Mở rộng cuộc gọi nhóm"
                onPointerDown={handleFloatingPointerDown}
                onPointerMove={handleFloatingPointerMove}
                onPointerUp={handleFloatingPointerUp}
                onKeyDown={(e) => {
    ...
    return (
        <Backdrop open sx={{ zIndex: 2000, color: "#fff", bgcolor: "rgba(2,6,23,0.94)" }}>
            ...
            >
                {isAudioCall && !isGroupCall ? <CallAvatarBackground src={peerAvatar} /> : null}
                ...
                <Box sx={{ flex: 1, position: "relative", zIndex: 1, minHeight: 0 }}>
                    ...
                    {/* Self-view PiP for 1-on-1 video */}
                    ...
                    {/* Top-left: back button + name + timer */}
                    <Box
                        sx={{
                            position: "absolute",
                            top: 16,
                    ...
                    >
                        ...
                        <Box sx={{ pt: 0.25 }}>
                            <Typography variant="subtitle1" fontWeight={800}>
                                {isGroupCall ? groupName : displayNameOf(peer, "Cuộc gọi")}
                            </Typography>
                            <Typography variant="body2" sx={{ opacity: 0.8 }}>
                                {state === "reconnecting" ? "Đang kết nối lại..." : timer}
                                {isGroupCall &&
                                    ` • ${participants.filter((p) => p.status === "joined").length} người`}
                            </Typography>
                        </Box>
                    </Box>
                </Box>
                ...
                <Stack
                    ...
                    sx={{
                        ...
                        py: 3,
                        background:
                            controlsBackground || alpha(muiTheme.palette.background.paper, 0.84),
                ...
                >
                    ...
                    <Tooltip
                        title={isGroupCall ? "Rời cuộc gọi" : "Kết thúc cuộc gọi"}
                        placement="top"
                    >
                        <IconButton
                            aria-label={isGroupCall ? "Rời cuộc gọi" : "Kết thúc cuộc gọi"}
                            onClick={onEnd}
                            sx={{
                                bgcolor: "error.main",
                                color: "#fff",
        ...
        <Tooltip title={label} placement="top">
            <IconButton
                ...
                sx={{
                    ...
                    width: 56,
                    height: 56,
                    "&:hover": { bgcolor: `color-mix(in srgb, ${color} 24%, transparent)` },
                }}
            >
                {children}
            </IconButton>
...
function PeerAvatar({
    ...
    return (
        <Avatar
            src={groupAvatar ?? resolveCdnUrl(peer?.avatar)}
...
function CallAvatarBackground({ src }: { src?: string }) {
    return (
        <>
            ...
            <Box
                aria-hidden
                sx={{ position: "absolute", inset: 0, bgcolor: "rgba(3,7,18,0.42)" }}
            />
        </>
    );
}

function displayNameOf(peer: Participant | undefined, fallback: string) {
