...
import { EpisodePanel } from "@pages/movie/components/player/EpisodePanel";
import { EpisodePopover } from "@pages/movie/components/player/EpisodePopover";
import { PlayerCenterControls } from "@pages/movie/components/player/PlayerCenterControls";
import { PlayerSpeedMenu } from "@pages/movie/components/player/PlayerSpeedMenu";
import { PlayerTimeline } from "@pages/movie/components/player/PlayerTimeline";
import { PlayerTopBar } from "@pages/movie/components/player/PlayerTopBar";
import { RatingBanner } from "@pages/movie/components/player/RatingBanner";
...
import type { CachedEpisode } from "@/services/movieBackendService";
import {
	getCachedEpisodes,
	getEpisodePositions,
	updateWatchPosition,
...
export const VideoPlayerDialog = ({
	...
	// Stable refs so event handlers always see latest values without re-attaching
	const resetOverlayTimerRef = useRef(resetOverlayTimer);
	resetOverlayTimerRef.current = resetOverlayTimer;
	const savePositionAndRefreshRef = useRef(savePositionAndRefresh);
	savePositionAndRefreshRef.current = savePositionAndRefresh;
	const movieSlugRef = useRef(movieSlug);
	movieSlugRef.current = movieSlug;
	const currentEpSlugRef = useRef(currentEpSlug);
	currentEpSlugRef.current = currentEpSlug;
	const ageRatingRef = useRef(ageRating);
	ageRatingRef.current = ageRating;
	const categoriesRef = useRef(categories);
	categoriesRef.current = categories;

	// Stable video event handlers — defined once, never recreated, always read from refs
	const videoHandlersRef = useRef({
		onPlay: () => {
			setPlaying(true);
			resetOverlayTimerRef.current();
			if (
				ageRatingRef.current ||
		...
		onTimeUpdate: () => {
			...
			if (Number.isFinite(dur) && dur > 0 && !video.paused) {
				const pct = video.currentTime / dur;
				for (const m of MILESTONES) {
					if (pct >= m && !firedMilestonesRef.current.has(m)) {
						firedMilestonesRef.current.add(m);
						savePositionAndRefreshRef.current(
							movieSlugRef.current ?? "",
							currentEpSlugRef.current ?? "",
							video.currentTime,
							dur,
						);
						break;
		...
		onWaiting: () => {
			const video = videoRef.current;
			if (video && !video.paused) setBuffering(true);
		},
	...
	const startHlsRef = useRef<
		...
		// biome-ignore lint/style/noNonNullAssertion: ref is assigned before first use
	>(null!);
		...
		(video: HTMLVideoElement, src: string, isFallback = false) => {
			...
			if (Hls.isSupported()) {
				...
				hls.on(Hls.Events.MANIFEST_PARSED, (_e, data) => {
					// Set duration from manifest immediately — before durationchange fires
	...
	// Attach listeners synchronously here — before startHls — so we never miss events.
	const setVideoRef = useCallback(
		(el: HTMLVideoElement | null) => {
			// Remove listeners from old element
			if (videoRef.current) {
				const h = videoHandlersRef.current;
				videoRef.current.removeEventListener("play", h.onPlay);
				videoRef.current.removeEventListener("pause", h.onPause);
				videoRef.current.removeEventListener("timeupdate", h.onTimeUpdate);
				videoRef.current.removeEventListener(
					"durationchange",
					h.onDurationChange,
				);
				videoRef.current.removeEventListener("volumechange", h.onVolumeChange);
				videoRef.current.removeEventListener("ratechange", h.onRateChange);
				videoRef.current.removeEventListener("waiting", h.onWaiting);
				videoRef.current.removeEventListener("stalled", h.onStalled);
				videoRef.current.removeEventListener("canplay", h.onCanPlay);
				videoRef.current.removeEventListener("playing", h.onPlaying);
			}
			videoRef.current = el;
			if (el) {
				// Attach listeners BEFORE startHls so no event is missed
				const h = videoHandlersRef.current;
				...
				el.addEventListener("volumechange", h.onVolumeChange);
				...
				el.addEventListener("canplay", h.onCanPlay);
				el.addEventListener("playing", h.onPlaying);
				setVideoMounted((n) => n + 1);
	...
	useEffect(() => {
		...
		queueMicrotask(() => setRatingBannerVisible(false));
		if (ratingBannerTimerRef.current)
			clearTimeout(ratingBannerTimerRef.current);
	...
	// Sync React state snapshot after the <video> element mounts.
	// Listeners are already attached in setVideoRef (synchronously), so we only
	// need to read whatever state the video already has at this point.
	useEffect(() => {
		...
		const load = async () => {
			...
			try {
				...
				if (!cancelled && cached && cached.length > 0) {
					...
					const serverMap = new Map<string, CachedEpisode[]>();
	...
	useEffect(() => {
		queueMicrotask(() => setActiveSeason(playingSeason));
	}, [playingSeason]);
		...
		>
			...
			>
				<Box
					...
					onClick={(e) => {
						// Only handle clicks directly on this box (not bubbled from controls)
						if (e.target === e.currentTarget) handleVideoAreaClick("left");
					}}
				>
					{/* Left half: single-click = toggle overlay, double-click = seek back (touch) or fullscreen (desktop) */}
					<Box
						sx={{
							position: "absolute",
							inset: 0,
					...
					/>
					{/* Right half: single-click = toggle overlay, double-click = seek forward (touch) or fullscreen (desktop) */}
					<Box
						sx={{
							position: "absolute",
							inset: 0,
					...
					{useNativePlayer && (
						<Box
							...
							onClick={() => handleVideoAreaClick("left")}
						>
							{/* Center controls */}
