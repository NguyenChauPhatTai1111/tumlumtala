...
export const ConversationItem = ({
    ...
    const showUnreadCount = unreadCount > 0 && !isLastMessageReaction && !isLastMessageActivity;
    const showReactionDot = (isLastMessageReaction || isLastMessageActivity) && unreadCount > 0;
    ...
    const hasDraft =
        !hasSending &&
        Boolean(draftText?.trim() || draftImageCount || draftVideoCount || draftFileCount);
    const draftTotalAttachments =
        (draftImageCount ?? 0) + (draftVideoCount ?? 0) + (draftFileCount ?? 0);

    const isLastMessageMine = conversation.last_message_sender_id === currentUserId;
    const readRecipients = useMemo(() => {
        ...
        return (conversation.participants ?? []).filter(
            (p: Participant) =>
                p.id !== currentUserId && p.last_read_seq != null && p.last_read_seq >= threshold,
        );
    }, [
        isLastMessageMine,
        hasDraft,
    ...
    return compact ? (
        <Tooltip title={getConversationDisplayName(conversation, currentUserId)}>{content}</Tooltip>
    ) : (
        content
    );
};
