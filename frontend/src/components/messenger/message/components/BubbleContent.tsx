...
import { formatFileSize, formatVideoDuration } from "@components/messenger/utils/format";
...
export function isFlagMsg(message: Message): {
    ...
    const msgType = String(message.message_type ?? "")
        ...
        .toLowerCase();
    const isFlag =
        msgType === "emoji" && (message.emoji_source_type === "flag" || /^[a-zA-Z]{2}$/.test(raw));
    ...
    if (type === "file") {
        ...
        return (
            ...
            >
                ...
                <Typography variant="caption" noWrap sx={(theme) => ({ ...textSx(theme), mt: 0 })}>
...
export const BubbleContent = ({
    ...
    const fileExt = isFileMessage ? getFileExtFromContent(message.content, fileOriginalName) : "";
    ...
    const callStatusLabel = getCallStatusLabel(callMeta?.status, callMeta?.duration_seconds);
    const callTime = isCallMessage
        ? new Date(message.created_at).toLocaleTimeString("vi-VN", {
              hour: "2-digit",
              minute: "2-digit",
          })
        : "";
    const callStatus = String(callMeta?.status ?? "").toLowerCase();
    const isMissedCall = callStatus === "missed";
    const isUnsuccessfulCall = ["missed", "rejected", "cancelled", "failed"].includes(callStatus);
                ...
                {isActivityMessage ? (
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                {isUnsuccessfulCall ? (
                                    <CallEndIcon sx={{ fontSize: 19, color: "#fff" }} />
                                ) : isVideoCallMessage ? (
                                    <VideocamIcon sx={{ fontSize: 19, color: "#fff" }} />
                                ) : (
                                    <CallIcon sx={{ fontSize: 19, color: "#fff" }} />
                                )}
                            </Box>
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                                <Typography
                                ...
                                >
                                    {isMissedCall ? `Nhỡ ${callTitle.toLowerCase()}` : callTitle}
                                </Typography>
                                <Typography
                                    variant="caption"
