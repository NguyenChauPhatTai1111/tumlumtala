...
func (uc *GetUserConversationsUseCase) Execute(ctx context.Context, userID uint, filter utils.QueryFilter) (*utils.Paginator[conversationDTO.ConversationDTO], error) {
	...
	for i, c := range items {
		dto := conversationDTO.ConversationDTO{
			...
			CustomIncomingBubbleColor: c.CustomIncomingBubbleColor,
			CustomOutgoingBubbleColor: c.CustomOutgoingBubbleColor,
			CustomIncomingTextColor:   c.CustomIncomingTextColor,
			CustomOutgoingTextColor:   c.CustomOutgoingTextColor,
