SET @status_exists = (
    ...
    WHERE table_schema = DATABASE() AND table_name = 'user_snapshots' AND column_name = 'status'
);
SET @add_status_sql = IF(
    @status_exists = 0,
    'ALTER TABLE user_snapshots ADD COLUMN status ENUM(''active'', ''inactive'') NOT NULL DEFAULT ''active'' AFTER role',
    'DO 0'
);
